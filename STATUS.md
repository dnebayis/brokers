...
2. Complete the representative testnet transfer → deactivation → reactivation check after the successful claim cycle, proving that only the transferred token's accounting changes. The lifecycle logic is proven in `contracts/test/Integration.t.sol` and `ScaleLifecycle.t.sol` (both pass, 2026-08-16). The live-chain demonstration is scripted in `indexer/transfer_reactivation_check.py`: it snapshots both tokens and `activeShares` before/after, transfers one activated Broker, asserts only that token deactivates and `activeShares` drops by one while the retained token is untouched, then reactivates from the new owner with a fresh 36,750 COAT burn and re-checks. Running it needs a source key holding two activated Brokers and a new-owner key funded with COAT + gas, so it executes wherever those testnet keys live.
