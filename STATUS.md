...
- **Stocks:** the route-ready universe is **23** independently fork-probed routes — 5 V1 (AAPL, AMD, AMZN, COIN, CRCL) plus 18 V2 (NVDA, TSLA, MSFT, GOOGL, META, PLTR, ORCL, MU, INTC, CRWV, SNDK, USAR, SPCX, QQQ, SGOV, SLV, SPY, USO), the latter all passing the mainnet-fork route probe (CI run 32066225360). The 194-address Robinhood canonical list is discovery data, not a purchase obligation. A new asset requires canonical-address, route, liquidity, feed and fork-probe verification before promotion into `route-ready.mainnet.json`; unverified canonical assets are excluded, never approximated. Adding a route is a post-deploy owner op (`setRoute` + `setStockFeed`), no redeploy.
