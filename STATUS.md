...
3. **Clean mainnet-fork release report.** The partial run (blocker-3 evidence above) is 4/6; the two failures are a weekend equity-feed staleness artifact, not a regression. A clean 6/6 needs a US-market-hours weekday fork with fresh feeds; the public RPC prunes historical state, so this final run needs an archive RPC or a live market-hours run.
