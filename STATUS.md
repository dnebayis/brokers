...
3. **Clean mainnet-fork release report.** Weekday live run (2026-08-17, fresh feeds) passes 5/6:
   ForkMainnet, ForkLaunch, ForkHook, ForkFullSystem and ForkStockRoutes — the earlier weekend
   `BadFeed()` failures were stale-feed artifacts and are gone. ForkScaleClaims cannot complete on the
   public RPC (`metadata is not found` — it prunes historical state during the heavy 356-tx run), not
   a code/feed failure. Capture the full 6/6 by running `mainnet-fork-release.yml` against the archive
   `RH_MAINNET_RPC_URL`. Deploy scripts now compile with the default pipeline (`forge build` clean).
...
5. **Production identities and provider credentials.** Only a deployer key exists today. Before mainnet: create the owner as a **hardware wallet** (Ledger/Trezor) and separate keeper and oracle-signer keys; the deployer hands ownership to the hardware wallet 2-step and is then discarded. Set the indexer/keeper GitHub secrets (a Congress data API key — FMP or Unusual Whales — plus `UPDATER_PRIVATE_KEY`, `KEEPER_PRIVATE_KEY`, `RH_RPC_URL` and the deployed addresses). Keep Vercel on `NEXT_PUBLIC_NETWORK=testnet` until a verified mainnet manifest exists; the build already refuses a mainnet config with placeholder addresses or a leaked key.
