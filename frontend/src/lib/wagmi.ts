...
import { ACTIVE_NETWORK, rhMainnet, rhTestnet, RPC_HTTP } from "./chains";
...
// multicall (below) and React-Query caching, per-user RPC load drops ~10x — what lets the
// app serve hundreds of concurrent users on a dedicated Alchemy endpoint without throttling.
const transport = fallback(RPC_HTTP.map((url) => http(url, { batch: { wait: 16 } })));
