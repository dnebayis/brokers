...
/// Deploys BrokerRendererV3 pointed at the deployed v1 renderer and mirrors v1's live wiring
...
///   forge script script/DeployRendererV3.s.sol --rpc-url $RPC --private-key $PK --broadcast
contract DeployRendererV3 is Script {
    function run() external {
        ...
        BrokerRendererV3 v2 = new BrokerRendererV3(owner, IBrokerRendererV1(v1Addr));
        v2.setBroker(IBrokerState(brokerAddr));
        v2.setStockTokens(toks, syms);
        v2.setCoat(coatAddr);
        vm.stopBroadcast();

        console2.log("BrokerRendererV3:", address(v2));
