...
/// @title BrokerRendererV3
/// @notice Same picture, pure traits. The image is rendered from the v1 renderer's `bitmapOf`
///         with byte-identical SVG code, and `attributes` carries ONLY the seven fixed art traits,
///         exactly as v1 emits them, so rarity is the real trait distribution and nothing else.
///         Live state (Active/Inactive, stock in the Broker wallet, COAT inside) is still in the
///         metadata, but outside `attributes`: as the tail of the description and as a separate
///         `live` object that marketplaces display or ignore but never score. v2 had tried
///         `display_type: number` for the live figures; OpenSea counted them anyway.
contract BrokerRendererV3 is Ownable2Step {
    ...
    function _json(uint256 tokenId) internal view returns (string memory) {
        ...
        (string memory liveText, string memory liveJson) = _live(tokenId);
        return string.concat(
            ...
            image,
            '","attributes":',
            _attributes(v1.traitsOf(tokenId)),
            ',"live":',
            liveJson,
            "}"
        );
